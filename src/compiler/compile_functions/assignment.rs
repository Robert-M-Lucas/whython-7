    ...
    match &line[1].0 {
        BasicSymbol::Assigner(assigner) => {
            ...
            let (original_variable, variable) = if is_ref {
                ...
                lines.push(Line::DynFromCopy(variable.0, non_ref, name_handler.type_table().get_type_size(new_type)?));
            ...
            if is_ref {
                lines.push(Line::DynToCopy(variable.0, original_variable.unwrap().0, name_handler.type_table().get_type_size(variable.1)?));
            }
